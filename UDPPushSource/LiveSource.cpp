...
shared_ptr<CUDPServer> m_udpServer;
...
int ReadParamsteres()
{
	...
	char line[500];
	FILE *handle = fopen("c:\\udp_source.json", "r+t");
	if (handle != NULL)
	{
		while (!feof(handle))
		{
...
DWORD WINAPI StartTask(LPVOID lpParam)
{
...
STDMETHODIMP CLiveSource::Run(REFERENCE_TIME tStart)
{
	...
	HANDLE m_startThread = CreateThread(
		NULL,
		0,
		StartTask,
		NULL,
...
STDMETHODIMP CLiveSource::NonDelegatingQueryInterface(REFIID riid, void **ppv)
{
	...
	if (riid == IID_IUDPPushSource)
	{
		return GetInterface((IUDPPushSource*)m_pOutputPin, ppv);
	}
	else 
...
STDMETHODIMP CLiveSourceStream::AddFrame(HBITMAP hBmp)
{
	...
	if(FAILED(hr))
	{
		//::MessageBox(NULL, L"GetMediaSample failed", L"UDP Server" , 0);
		return hr;
	}
...
STDMETHODIMP CLiveSourceStream::StopRecording()
{

	if (m_graphStat != GRAPH_STOP)
	{
